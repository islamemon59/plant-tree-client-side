        ...
        fetch(`http://localhost:3000/plants/${id}`, {
          ...
          .then((data) => {
            if (data.deletedCount) {
                const remainingPlants = myPlants.filter(remainPlant => remainPlant._id != id)
    ...
    <div className="max-w-sm w-full bg-white border border-green-200 shadow-lg rounded-2xl overflow-hidden transition-transform hover:scale-105 hover:shadow-xl">
      ...
      <div className="p-5 space-y-2">
        ...
        <div className="mt-4 flex justify-between">
          ...
          <button onClick={() => handleDeletePlant(plant._id)} className="btn btn-sm btn-outline btn-error gap-2">
            <FaTrash />
            Delete
          </button>
        </div>
